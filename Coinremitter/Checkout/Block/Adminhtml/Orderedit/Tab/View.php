...
class View extends \Magento\Backend\Block\Template implements \Magento\Backend\Block\Widget\Tab\TabInterface
{
    ...
    {
        if ($this->getInvoice()->getInvoiceId() || $this->getInvoice()->getAddress()) {
            return true;
        } else {
            return false;
        }
    ...
    public function getDetail()
    {
        ...
        $sql = "SELECT coin, address, expire_on, invoice_id, created_at, status FROM coinremitter_payment WHERE order_id=" . $orderId;
        $result = $connection->fetchAll($sql);
        $coin = $result[0]['coin'];
        $address = $result[0]['address'];
        $expire_on = $result[0]['expire_on'];
        $created_at = date("M d, Y H:i:s", strtotime($result[0]['created_at']));
        $invoice_id = $result[0]['invoice_id'];
        $order_status = $order->getStatus();
        $payment_status = $result[0]['status'];

        $resultData['expire_on'] = isset($expire_on) && $expire_on != '' ? date("M d, Y H:i:s", strtotime($expire_on)) . " (UTC)" : '-';
        ...
        if (isset($invoice_id) && $invoice_id != "") {
            ...
            $resultData['created_at'] = $created_at;
            ...
            $sql = "SELECT co.address, co.crp_amount, co.payment_status, cp.coin, cp.base_currancy FROM coinremitter_order as co, coinremitter_payment as cp where co.address=cp.address AND co.order_id=" . $orderId;
            $result = $connection->fetchAll($sql);
            $orderData = $result[0];
            ...
            $resultData['coin'] = $orderData['coin'];
            $resultData['base_currancy'] = $orderData['base_currancy'];
            $resultData['created_at'] = $created_at . " (UTC)";

            $resultData['transactions'] = array();
            $total_paid = 0;
            $notconfirm_paid = 0;
            $sql = "SELECT transaction_id, explorer_url, paid_amount, confirmations, paid_date, txId FROM coinremitter_webhook WHERE address='" . $orderData['address'] . "'";
            $result = $connection->fetchAll($sql);
            ...
            if ($result) {
                ...
                foreach ($webhookData as $webhook) {
                    ...
                    $data = array();
                    $data['transaction_id'] = $webhook['txId'];
                    $data['explorer_url'] = $webhook['explorer_url'];
                    $data['amount'] = $webhook['paid_amount'];
                    $data['confirmations'] = $webhook['confirmations'];
                    $data['paid_date'] = $webhook['paid_date'];
                    array_push($resultData['transactions'], $data);
                }
            }
            if (strtolower($order_status) == 'canceled') {
