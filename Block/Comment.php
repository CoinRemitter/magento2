...
class Comment extends \Magento\Framework\View\Element\Template
{
    ...
    public function getDetail()
    {
        ...
        if (empty($coinremitterOrder['transaction_meta']) && $expire_on != "") {
            ...
            if ($date_diff < 1 && $statusCode['expired'] != $coinremitterOrder['order_status']) {
                ...
                $total_fiat_paid = number_format(
                    ($total_paid * $coinremitterOrder['fiat_amount']) / $coinremitterOrder['crypto_amount'],
                    2,
                    '.',
                    ''
                );
                $sql = "UPDATE `coinremitter_orders` SET " .
                    "`paid_crypto_amount`=" . $transactionData['confirm_amount'] . "," .
                    "`paid_fiat_amount`=" . $total_fiat_paid . "," .
                    "`order_status`=" . $expiryStatus . "," .
                    "`transaction_meta`='" . json_encode($allTrx) . "' " .
                    "WHERE payment_address='" . $coinremitterOrder['payment_address'] . "'";
        ...
        } elseif ($total_paid > $coinremitterOrder['crypto_amount']) {
            $status = $statusCode['over_paid'];
        } elseif ($total_paid != 0 && $total_paid < $coinremitterOrder['crypto_amount']) {
    ...
    public function getTransactionsByAddress($param)
    {
    ...
    {
        $_val = $this->scopeConfig->getValue(
            $_env,
